...
{
	unsigned char const * data = reinterpret_cast<unsigned char const *>(input.c_str());
...
bool decompress_packet(unsigned char const * input, unsigned int input_size, unsigned char * output_buffer, unsigned int output_buffer_size, unsigned int & data_size)
{
	unsigned int a, b, c, d;
	unsigned int maximum_count, index, count;
	int size;

	b = 0;

	size = input_size;
	unsigned char const * input_pointer = input;

	maximum_count = output_buffer_size;
	unsigned char * output_pointer = output_buffer;
	count = 0x20;
	...
	while(true)
	{
		 ...
		 if(count > 0x20)
		 {
			  data_size = output_buffer_size - maximum_count;
			  return true;
		 }
		 ...
		 *output_pointer = (unsigned char)a;
		 output_pointer++;
...
std::string decompress_packet(std::string const & input)
{
	...
	std::size_t size = determine_game_packet_size(input, header_size);

	if(!decompress_packet(reinterpret_cast<unsigned char const *>(input.c_str() + header_size), static_cast<unsigned int>(size), reinterpret_cast<unsigned char *>(output_buffer), static_cast<unsigned int>(sizeof(output_buffer)), data_size))
	{
		std::cout << "Failed to decompress data, buffer_valuefer too small" << std::endl;
		return "";
	}

	return std::string(reinterpret_cast<char *>(output_buffer), static_cast<std::size_t>(data_size));
}

unsigned int compression_table[] =
{
...
unsigned int create_game_packet_size_header(unsigned int size, unsigned char * output)
{
	if(size > 238)
	{
		size += 2;
		size |= 0xF000;
		output[0] = (unsigned char)(size >> 8);
		output[1] = (unsigned char)(size & 0xFF);
		return 2;
	}
	else
	{
		output[0] = (unsigned char)(size + 1);
		return 1;
	}
}

unsigned int compress_packet(unsigned char const * input, unsigned int input_size, unsigned char * output)
{

	unsigned int a, e, buffer_value = 0;
	int count = 0;
	...
	while(input_size != 0)
	{
		...
		if(e)
		{
			buffer_value |= ((a & 0xFF) << (8 - e)) << (24 - count);
			count += e;
		}

		while(count > 8)
		{
			*output = buffer_value >> 24;
			output++;
			count -= 8;
			buffer_value <<= 8;
		}
	...
	return (unsigned int) (output - output_pointer);
}

std::string compress_packet(std::string const & input)
{
	unsigned char * buffer = new unsigned char[input.length()];
	unsigned int output_size = compress_packet(reinterpret_cast<unsigned char const *>(input.size()), static_cast<unsigned int>(input.length()), buffer);
	return std::string(reinterpret_cast<char *>(buffer), static_cast<std::size_t>(output_size));
}
